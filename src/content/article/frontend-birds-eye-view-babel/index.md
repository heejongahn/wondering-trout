...
1. 구문 분석*parsing*: 소스 코드를 읽어 [추상 구문 트리](https://ko.wikipedia.org/wiki/%EC%B6%94%EC%83%81_%EA%B5%AC%EB%AC%B8_%ED%8A%B8%EB%A6%AC)로 변환하는 단계.
2. 변환*transformation*: 정해진 규칙대로 추상 구문 트리에 변경을 가하는 단계.
3. 출력*printing*: 변경이 가해진 추상 구문 트리를 다시 코드로 출력하는 단계.
